      ...
      class Web
        ...
        def autoload
          ...
          if defined?(Gamefic::Autoload)
            Gamefic.logger.info 'Generating autoload.rb file for Opal'
            autoload_path = File.join(absolute_path, 'autoload.rb')
            autoload_file = <<~FILE
              # frozen_string_literal: true

              # This file is automatically generated by the Gamefic SDK.

              #{Gamefic::Autoload.encode_all.join("\n")}
            FILE
            File.write(autoload_path, autoload_file)
          else
            Gamefic.logger.info 'Gamefic::Autoload is not enabled.'
          end
        end
